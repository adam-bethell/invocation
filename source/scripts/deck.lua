...
local whiteShieldImageLarge = gfx.image.new(30,30, gfx.kColorWhite)
gfx.pushContext(whiteShieldImageLarge)
    whiteShieldImage:drawScaled(0, 0, 2)
gfx.popContext()
local blackShieldImage = gfx.image.new("images/black_shield")
assert(blackShieldImage)
local blackShieldImageLarge = gfx.image.new(30,30, gfx.kColorWhite)
gfx.pushContext(blackShieldImageLarge)
    blackShieldImage:drawScaled(0, 0, 2)
gfx.popContext()
...
function Deck:init()
    ...
    self.whiteShieldSelectorSprite = gfx.sprite.new(whiteShieldImageLarge)
    self.whiteShieldSelectorSprite:moveTo(self.xs[self.whiteShield], 25)
    self.whiteShieldSelectorSprite:setZIndex(5)
    self.whiteShieldSelectorSprite:setVisible(false)
    self.whiteShieldSelectorSprite:add()
    self.blackShieldSelectorSprite = gfx.sprite.new(blackShieldImageLarge)
    self.blackShieldSelectorSprite:moveTo(self.xs[self.blackShield], 25)
    self.blackShieldSelectorSprite:setZIndex(5)
    self.blackShieldSelectorSprite:setVisible(false)
    self.blackShieldSelectorSprite:add()
...
function Deck:setPlayer(player)
    self.player = player
end

function Deck:giveFocus()
    self.hasFocus = true
    
    if (self.player == 1) then
        self.whiteShieldSelectorSprite:setVisible(true)
    else 
        self.blackShieldSelectorSprite:setVisible(true)
    end
end

function Deck:getFocus()
...
function Deck:update()
    if (self.hasFocus) then
        ...
        if (change ~= 0) then
            if (self.player == 1) then
                self.whiteShield = math.min(math.max(1, self.whiteShield + change), 3)
                self.whiteShieldSelectorSprite:moveTo(self.xs[self.whiteShield], 25)
            elseif (self.player == 2) then
                self.blackShield = math.min(math.max(1, self.blackShield + change), 3)
                self.blackShieldSelectorSprite:moveTo(self.xs[self.blackShield], 25)
            end
        end
